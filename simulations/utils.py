...
class RiskScoreCalculator:
    ...
    def calculate_scenario_risk_score(
        ...
        if financial_impact > 0:
            financial_score = min(
                40,
                10 + (10 * math.log10(float(financial_impact) / 100000))
            )
        else:
            financial_score = 0
...
class CascadeAnalyzer:
    ...
    def trace_dependency_chain(
        ...
        """
        Trace vendor dependency chain recursively.

        Returns a list of (vendor, depth, impact_multiplier) tuples where
        depth is the number of hops from the root vendor (root itself = 0).

        Bug fixed: previously all direct dependencies were hardcoded to depth=1
        regardless of how deep in the recursion we were. Now depth is passed
        explicitly so each level is reported correctly.
        """
        if visited is None:
            visited = set()

        return CascadeAnalyzer._trace(vendor, max_depth, visited, current_depth=0)
    ...
    def _trace(
        ...
        if vendor.id in visited or remaining_depth < 0:
            return []

        visited.add(vendor.id)

        # Decay factor: 0.8 per hop (20% reduction per dependency level)
        decay = 0.8
        impact_multiplier = decay ** current_depth

        chain = [(vendor, current_depth, impact_multiplier)]

        for dep_vendor in vendor.dependent_vendors.all():
            if dep_vendor.id not in visited:
                sub_chain = CascadeAnalyzer._trace(
                    dep_vendor,
                    remaining_depth - 1,
                    visited,
                    current_depth + 1,
...
class MonteCarloSimulator:
    ...
    def run_iterations(
        ...
        """
        Run Monte Carlo iterations using a log-normal distribution.

        Using log-normal instead of a clipped normal distribution because:
        - Cyber losses are right-skewed — rare but catastrophic outcomes matter
        - A clipped normal at ±3*sigma artificially suppresses tail risk,
          which contradicts the FAIR framework underpinning this project
        - Log-normal is the standard distribution used in CRQ platforms

        The `variance` parameter is the sigma of the underlying normal
        (default 0.30 = 30%, wider than the previous 15% to capture realistic
        tail risk). The median of the resulting distribution equals
        ...
        samples = np.random.lognormal(mean=mu, sigma=sigma, size=iterations)
        results = samples * baseline_value
        ...
        return {
            ...
            'distribution_sample': results[:100].tolist(),
        }

    @staticmethod
    def analyze_risk_distribution(monte_carlo_results: Dict) -> Dict:
...
class ReportGenerator:
    ...
    def generate_executive_summary(simulation_result) -> Dict:
        ...
        if simulation_result.total_financial_impact > 1_000_000:
            findings.append("Financial impact exceeds $1 million - immediate mitigation required")

        if simulation_result.downtime_hours > 48:
            findings.append(
                f"Extended downtime of {simulation_result.downtime_hours:.0f} hours expected"
            )

        if simulation_result.recovery_complexity in ['high', 'very_high']:
            findings.append("Complex recovery process - specialised expertise required")

        if len(simulation_result.cascading_vendor_impacts) > 0:
            findings.append(
                f"{len(simulation_result.cascading_vendor_impacts)} dependent vendors affected"
            )
    ...
    def _generate_recommendations(simulation_result) -> List[str]:
        ...
        if simulation_result.regulatory_costs > 100_000:
            recommendations.append(
                "Strengthen compliance controls to minimise regulatory exposure"
            )

        if simulation_result.downtime_hours > 24:
            recommendations.append(
                "Implement business continuity plan with alternate vendor options"
            )

        if simulation_result.recovery_complexity == 'very_high':
            recommendations.append(
                "Establish incident response retainer with specialised security firm"
            )

        if len(simulation_result.cascading_vendor_impacts) > 2:
            recommendations.append(
                "Reduce vendor dependencies or implement redundancy"
            )

        critical_processes = simulation_result.affected_processes.filter(
            criticality_level__gte=4
        )
        if critical_processes.exists():
            recommendations.append(
                "Critical processes affected - establish backup service providers"
            )
